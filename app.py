...
def format_ai_understanding(data):
    """Convert appointment data into a user-friendly display."""
    ...
    lines = [
        "📋 Appointment Details",
        "",
        f"Action: {intent_labels.get(intent, intent.title())}"
    ]
    ...
    if intent == "book":
        ...
        if data.get("appointment_date"):
            lines.append(f"Date: {data['appointment_date']}")
        if data.get("appointment_time"):
            lines.append(f"Time: {data['appointment_time']}")
        if data.get("purpose"):
        ...
        if data.get("appointment_id"):
            lines.append(f"Appointment ID: {data['appointment_id']}")

    elif intent == "reschedule":
        if data.get("appointment_id"):
            lines.append(f"Appointment ID: {data['appointment_id']}")
        if data.get("new_appointment_date"):
            lines.append(f"New Date: {data['new_appointment_date']}")
        if data.get("new_appointment_time"):
            lines.append(f"New Time: {data['new_appointment_time']}")

    elif intent == "list":
        lines.append("Request: Show my appointments")
